  ...
  const loadStatus = async () => {
    try {
      const [tally, api, sync] = await Promise.all([
        window.electronAPI?.getTallyStatus?.() || Promise.resolve(null),
        window.electronAPI?.getApiStatus?.() || Promise.resolve(null),
        window.electronAPI?.getSyncStatus?.() || Promise.resolve(null),
      ]);

      if (tally) setTallyStatus(tally);
    ...
    <div className="bg-card p-4 rounded-lg border border-border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${getStatusColor(tallyStatus?.isOnline)}`} />
          <div className="flex-1">
            ...
            {tallyStatus?.errorMessage && !tallyStatus.isOnline && (
              <div className="text-xs text-red-500 mt-1">{tallyStatus.errorMessage}</div>
            )}
          </div>
        </div>
        ...
        {/* API Status */}
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${getStatusColor(apiStatus?.isOnline)}`} />
          <div className="flex-1">
            ...
            {apiStatus?.errorMessage && !apiStatus.isOnline && (
              <div className="text-xs text-red-500 mt-1">{apiStatus.errorMessage}</div>
            )}
          </div>
        </div>
        ...
        {/* Sync Status */}
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${getSyncStatusColor(syncStatus?.status)}`} />
          <div className="flex-1">
            <div className="text-sm font-medium text-foreground">Sync</div>
            <div className="text-xs text-muted-foreground">
              {syncStatus?.isRunning ? 'Running' : syncStatus?.status === 'error' ? 'Error' : 'Idle'}
            </div>
