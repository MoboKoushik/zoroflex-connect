...
export const Dashboard: React.FC = () => {
  ...
  const [logType, setLogType] = useState<'system' | 'api' | 'tally-sync' | 'all'>('all');
  ...
  useEffect(() => {
    ...
    if (window.electronAPI?.onSyncCompleted) {
      window.electronAPI.onSyncCompleted((data: any) => {
        setIsSyncing(false);
        setSyncStatus(data?.error ? `Sync failed: ${data.error}` : 'Sync completed');
        if (!data?.error) {
  ...
  const handleSyncStart = (type: 'full' | 'fresh') => {
    setIsSyncing(true);
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ 
          ...
          {currentPage === 'dashboard' && (
            <>
              <h1 style={{ fontSize: '28px', marginBottom: '24px', color: 'var(--text-primary)' }}>Dashboard</h1>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  ...
                  <SyncControls onSyncStart={handleSyncStart} onSyncComplete={handleSyncComplete} />
                </div>
            ...
            <div style={{ height: 'calc(100vh - 120px)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    background: logType === 'all' ? 'var(--bg-tertiary)' : 'var(--bg-secondary)',
                ...
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    background: logType === 'system' ? 'var(--bg-tertiary)' : 'var(--bg-secondary)',
                    border: '1px solid var(--border-color)',
                    borderRadius: '4px',
                    color: 'var(--text-primary)',
                    cursor: 'pointer',
                ...
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    background: logType === 'api' ? 'var(--bg-tertiary)' : 'var(--bg-secondary)',
                    border: '1px solid var(--border-color)',
                    borderRadius: '4px',
                    color: 'var(--text-primary)',
                    cursor: 'pointer',
                ...
                <button
                  ...
                  style={{
                    padding: '8px 16px',
                    background: logType === 'tally-sync' ? 'var(--bg-tertiary)' : 'var(--bg-secondary)',
                    border: '1px solid var(--border-color)',
                    borderRadius: '4px',
                    color: 'var(--text-primary)',
                    cursor: 'pointer',
